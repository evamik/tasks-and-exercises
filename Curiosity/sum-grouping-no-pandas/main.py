...
def read_from_file(filename, separator=";", verbose=True) -> DataFrame:
    df = DataFrame()
    try:
...
def case_one(df: DataFrame) -> DataFrame:
    ...
    df = df.groupby("Darbuotojas").sum().rename(
        columns={"Alga": "Suma"})
...
def case_two(df: DataFrame) -> DataFrame:
    ...
    df = df.groupby(["Darbuotojas", "Tipas"]).sum().rename(
        columns={"Alga": "Suma"})
