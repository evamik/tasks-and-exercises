    ...
    public class Main
    {
        ...
        public static void CalculateCosts()
        {
            ...
            foreach(SubscribtionsData subData in DataList)
            {
                foreach(Subscribtion sub in subData.Subscribtions)
                {
                    Subscriber subscriber = SubToSubscriber(sub);
                    double cost = sub.PeriodLength * sub.Amount * SubToIssue(sub).MonthlyCost;
                    if (subscriber == null)
                    {
                        subscriber = new Subscriber(sub.Surname, sub.Address);
                        subscriber.AddCost(cost);
        ...
        public static void GetOrderDates(DateTime startDate, DateTime endDate, int month)
        {
            ...
            foreach(SubscribtionsData subData in DataList)
            {
                if(subData.Date >= startDate && subData.Date <= endDate)
                {
                    foreach(Subscribtion sub in subData.Subscribtions)
                    {
                        if(month >= sub.PeriodStart && month <= sub.PeriodStart + sub.PeriodLength)
                        {
                            DateTime date = new DateTime(subData.Date.Year, month, 1);
                            Subscriber subscriber = SubToSubscriber(sub);
                            if (!subscriber.OrderedDates.Contains(date))
                                subscriber.AddDate(date);
                        }
                    }
                }
            }
        ...
        {
            int index = IssueList.IndexOf(new Issue(sub.IssueCode, "", 0));
            if (index != -1)
        ...
        {
            int index = SubscriberList.IndexOf(new Subscriber(sub.Surname, sub.Address));
            if (index != -1)
                return SubscriberList[index];
            return null;
        }
