// Developed to work on Scalatron
...
object ControlFunction
{
    ...
    def nodeIndex(node: Node, nodes: ArrayBuffer[Node]) : Int = {
    ...
    def shortestNode(nodes : ArrayBuffer[Node]) : Node = {
        var shortestDistance = 999999999
        var index = -1
        for (i <- 0 until nodes.length){
            if(shortestDistance > nodes(i).shortestDistance){
                shortestDistance = nodes(i).shortestDistance
                index = i
            }
        }
        nodes(index)
    ...
    def forMaster(bot: Bot) {
        ...
        var nodes = new Array[Node](31*31)
        var line = ""
        var index = 0
        var masterIndex = -1
        ...
        for( y <- 0 until 31){
            for( x <- 0 until 31){
                ...
                val entityString = bot.view.cells.charAt(x+y*31).toString
                val cost = entityString match {
                    case "W" => 1000 //wall
                    case "?" => 100 //can't see
                ...
                if(y > 0){
                    node.addNeighbour(nodes(posToIndex(node.x, node.y-1)))
                    if(x > 0) {
                        node.addNeighbour(nodes(posToIndex(node.x-1, node.y-1)))
                    }
                    if(x < 30){
                        node.addNeighbour(nodes(posToIndex(node.x+1, node.y-1)))
                    }
                ...
                if(x > 0) {
                    node.addNeighbour(nodes(posToIndex(node.x-1, node.y)))
                }
                node.index = index
                nodes(index) = node
                index+=1
...
case class Node(x: Int, y: Int) {
    var index = -1
    var gCost = 1
    var neighbours : ArrayBuffer[Node] = ArrayBuffer[Node]()
