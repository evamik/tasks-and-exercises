...
def ScrollTextBox(w, h):
    ...
    T = Text(frame1, width=w, height=h, wrap=NONE,
        ...
        for i in range(0, siz[0]):
            ...
            if len(siz) > 1:
                for j in range(0, siz[1]):
                    T.insert(END, "%12g   " % A[i, j])
            else:
                T.insert(END, "%12g   " % A[i])
    else:
        T.insert(END, "%12g   " % A)
    T.yview(END)
    T.update()
...
def Pavirsius(X, Y, LFF):
    ...
    for i in range(0, siz[0]):
        for j in range(0, siz[1]):
            Z[i, j, :] = LFF([X[i][j], Y[i][j]]).transpose()
    return Z
# *****************************************************************
# *****************************************************************
...
def jetColormapValueToRGB(value):
    ...
    for i in range(0, N-1):
        a = 1.0*i / (N - 1)
        b = 1.0*(i+1) / (N - 1)  # print(a); print(b)
        if (value >= a) & (value <= b):
            rr = (value - a) / (b - a)
            for j in range(0, 3):
                rgb.append(
                    np.float(jetColors[i][j] * (1 - rr) + jetColors[i + 1][j] * rr))
            break
    return rgb
# *****************************************************************
# *****************************************************************
