 ...
 * pvz. klasės Bike objektas, klasėje būtina perdengti metodus equals(Object o)
 * ir hashCode().
 *
...
public class HashMap<K, V> implements EvaluableMap<K, V> {
    ...
    public static final CollisionHandling DEFAULT_COLLISION_HANDLING = CollisionHandling.NONE;
    ...
    // Kolizijos tvarkymas
    protected CollisionHandling ch;
    //--------------------------------------------------------------------------
    ...
    public HashMap(int initialCapacity, float loadFactor, HashType ht) {
        this(initialCapacity, loadFactor, ht, DEFAULT_COLLISION_HANDLING);
    }

    public HashMap(int initialCapacity, float loadFactor, HashType ht, CollisionHandling ch){
    ...
    public boolean isCh(){
        if(ch == CollisionHandling.LINEAR_PROBING)
            return true;
        return false;
    }
    ...
    public void replaceAll(V oldValue, V newValue){
        if (oldValue == null || newValue == null) {
            throw new IllegalArgumentException("oldValue or newValue is null in replaceAll(V oldValue, V newValue)");
        }
    ...
    private void rehash(Node<K, V> node) {
        HashMap<K, V> newMap = new HashMap<>(table.length * 2, loadFactor, ht, ch);
    ...
    private int hash(K key, HashType hashType) {
        ...
        switch (hashType) {
            case DIVISION:
                hash = Math.abs(h) % table.length;
                break;
            case MULTIPLICATION:
                double k = (Math.sqrt(5) - 1) / 2;
                hash = (int) (((k * Math.abs(h)) % 1) * table.length);
                break;
            case JCF7:
                h ^= (h >>> 20) ^ (h >>> 12);
                h = h ^ (h >>> 7) ^ (h >>> 4);
                hash = h & (table.length - 1);
                break;
            case JCF8:
                h = h ^ (h >>> 16);
                hash = h & (table.length - 1);
            ...
            case LINEAR_PROBING:
                if(table[hash] != null)
                    for(int i = 1; i < table.length; i++){
                        int in = i+hash;
                        if(in >= table.length) in-=table.length;
                        if(table[in]==null){
                            hash = in;
                            break;
                        }
