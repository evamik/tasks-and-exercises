...
// http://www.java2s.com/Tutorials/Java/Java_Regular_Expression/0070__Java_Regex_Groups.htm    
    ...
    ***************************************************/
    String[] regexes = { "\\d+", // pradinė regex sąlyga
                        "-", // minuso atpažinimo sąlyga
                        "\\p{IsLatin}*\\p{IsLatin}", // a sąlyga
                        "\\p{IsLatin}*[\u0104-\u017E]+\\p{IsLatin}*", // b sąlyga
                        "\\(.+?\\)"}; // c sąlyga
    int index = 0;
    void regexExamples(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText("pirmas=1-;antras>-22:.(.ką,.1))trečias:333viskas21,ąsotėlis..," + nL);
        String text = readLastLine();
        ta2.appendText(regexes[index] + " : " + text + nL); // pakartojame dešinėje
        Pattern pat = Pattern.compile(regexes[index]);      // pirma dalis - tai Pattern
        Matcher m = pat.matcher(text);            // kita dalis - apdorojimui
        while (m.find())                     // kol randamas šablonas
            ta2.appendText(m.group() + nL);  // pateikiamas jo atitikmuo
        index = (index < regexes.length) ? index+1 : 0;
    }
