...
public class LinkedList<E extends Comparable<E>>
    ...
    public boolean contains(E e){
        ...
        for(int i = 0; i < size; i++){
            if(first.findNode(i).element.toString().equals(e.toString()))
                return true;
        }
    ...
    public void removeRange(int fromIndex, int toIndex){
        if(fromIndex < 0 || fromIndex > size-1)
            return;
        if(toIndex < fromIndex || toIndex > size-1)
            return;
        
        //for(int i = fromIndex; i < toIndex; i++)
        //    remove(fromIndex);
        
        Node<E> n = first.findNode(fromIndex-1);
        for(int i = 0; i < toIndex-fromIndex; i++){
            if(fromIndex == 0) removeFirst();
            else {
                n.next = n.next.next;
                size--;
            }
    ...
    public boolean add(int k, E e) {
        ...
        if(k == 0){
            ...
            Node<E> nl = first.findNode(k-1);
            Node<E> nl2 = nl.next;
            nl.next = new Node(e, nl2);
        }
        size++;
        return true;
    }
    ...
    public E set(int k, E e) {
        return first.findNode(k).element = e;
    }
