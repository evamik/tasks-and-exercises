...
import edu.ktu.ds.lab2.mikalauskas.Bike;
import edu.ktu.ds.lab2.mikalauskas.BikesGenerator;
import edu.ktu.ds.lab2.mikalauskas.SimpleBenchmark;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static ParsableSortedSet<Bike> bikesSet;
    private BikesGenerator bikesGenerator = new BikesGenerator();
    ...
    private void treeGeneration(String filePath) throws ValidationException {
        ...
        if (filePath == null) {
            bikesArray = bikesGenerator.generateShuffle(sizeOfGenSet, sizeOfInitialSubSet, shuffleCoef);
            paneParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            bikesSet.load(filePath);
            bikesArray = new Bike[bikesSet.size()];
            int i = 0;
            for (Object o : bikesSet.toArray()) {
                bikesArray[i++] = (Bike) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(bikesArray), new Random());
        }

        // Išmaišyto masyvo elementai surašomi i aibę
        bikesSet.clear();
        Arrays.stream(bikesArray).forEach(bikesSet::add);

        // Išvedami rezultatai
        // Nustatoma, kad eilutės pradžioje neskaičiuotų išvedamų eilučių skaičiaus
        KsGui.setFormatStartOfLine(true);
        KsGui.oun(taOutput, bikesSet.toVisualizedString(tfDelimiter.getText()),
    ...
    private void treeAdd() throws ValidationException {
        KsGui.setFormatStartOfLine(true);
        Bike bike = bikesGenerator.takeBike();
        bikesSet.add(bike);
        paneParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsGui.oun(taOutput, bike, MESSAGES.getString("setAdd"));
        KsGui.oun(taOutput, bikesSet.toVisualizedString(tfDelimiter.getText()));
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (bikesSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(bikesSet.size());
            Bike bike = (Bike) bikesSet.toArray()[nr];
            bikesSet.remove(bike);
            KsGui.oun(taOutput, bike, MESSAGES.getString("setRemoval"));
            KsGui.oun(taOutput, bikesSet.toVisualizedString(tfDelimiter.getText()));
        }
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void treeIteration() {
        ...
        if (bikesSet.isEmpty()) {
            ...
            KsGui.oun(taOutput, bikesSet, MESSAGES.getString("setIterator"));
        }
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws ValidationException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                bikesSet = new ParsableBstSet<>(Bike::new);
                break;
            case 1:
                bikesSet = new ParsableAvlSet<>(Bike::new);
                break;
            default:
                disableButtons(true);
