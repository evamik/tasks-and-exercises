...
public class BstSet<E extends Comparable<E>> implements SortedSet<E>, Cloneable {
    ...
    public SortedSet<E> headSet(E toElement, boolean inclusive){
        ...
        for(E el : this){
            if(c.compare(toElement, el) > 0)
                set.add(el);
        }

        if(inclusive && c.compare(toElement, toElement) == 0)
            set.add(toElement);
    ...
    public E last(){
        E ret = this.getMax(root).element;

        for(E el : this){
            if(c.compare(ret, el) < 0)
                ret = el;
        }
    ...
    public boolean removeAll(BstSet<? extends E> c){
        if(c == null){
            throw new IllegalArgumentException("Collection is null in removeAll(BstSet<? extends E> c)");
        }
    ...
    public boolean addAll(BstSet<? extends E> c){
        if(c == null){
            throw new IllegalArgumentException("Collection is null in addAll(BstSet<? extends E> c)");
        }
    ...
    public Set<E> subSet(E element1, E element2) {
        ...
        for(E el : this){
            if(c.compare(element1, el) <= 0)
                if(c.compare(element2, el) > 0)
                    set.add(el);
        }
    ...
    private class IteratorBst implements Iterator<E> {
        ...
        public void remove() {
            if(hasNext()){
                parent = stack.pop();
                BstSet.this.remove(parent.element);
            }
            else parent = null;
        }
